...
::: tip NOTE
Allocating objects or invoking the GC externally requires compiling with `--exportRuntime`. See: [Garbage collection](./garbage-collection.md).
:::
...
When compiling with `--exportRuntime`, the loader will expose the runtime interface (`__new`, `__pin`, `__unpin`, `__collect`) as well. See: [Garbage collection](./garbage-collection.md)
...
All of the above can be mixed with direct [memory](./memory.md) accesses on `myModule.exports.memory.buffer`, for instance by adhering to [class layout](./interoperability.md#class-layout).
